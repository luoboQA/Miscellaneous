/**
 * @brief 无序多重集合（Unordered Multiset）容器
 * 
 * 基于哈希表实现，元素可重复，自动去重（但允许多个相同元素）
 ...
 * - 链地址法：解决哈希冲突
 * 
 * 与 MyUnorderedSet（假设存在）的区别：
 * - MyUnorderedSet：元素唯一，插入重复元素会失败
 * - MyUnorderedMultiset：元素可重复，插入永远不会失败
 * 
 * 与 MyMultiset 的区别：
 * - MyMultiset：红黑树，O(log n)，元素有序
 * - MyUnorderedMultiset：哈希表，O(1) 平均，元素无序
 * 
 * 时间复杂度（平均）：
 * - 插入：O(1)
 * - 删除一个：O(1)
...
public:
     ...
     * 时间复杂度：O(1) 平均
     * 
     * 与 MyUnorderedSet（假设存在）不同，这里不会检查元素是否已存在
     * 相同值的节点会存储在同一个桶中
     */
     ...
     * 时间复杂度：O(k) 平均，k 为相同值的数量
     * 
     * 遍历该值所在桶中的所有节点，统计匹配的数量
     */
     ...
     * @brief 拷贝赋值
     * @param other 要拷贝的对象
     * @return MyUnorderedMultiset& 当前对象的引用
     * 时间复杂度：O(n)
 ...
 */
template<typename Key>
void swap(MyUnorderedMultiset<Key>& a, MyUnorderedMultiset<Key>& b) {
    a.swap(b);
}
