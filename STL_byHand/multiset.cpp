/**
 * @brief 多重集合（Multiset）容器
 * 
 * 基于红黑树实现，允许元素重复
 * 元素自动排序（升序），相同的元素在树中可以有多个副本
 * 
 * 与 MySet 的区别：
 * - MySet：元素唯一，插入重复元素会失败
 * - MyMultiset：元素可重复，插入永远不会失败
 * 
 * 红黑树性质：
 * 1. 每个节点是红色或黑色
 * 2. 根节点是黑色
...
private:
    ...
    /**
     * @brief 查找第一个 >= val 的节点（lower_bound）
     * @param val 要查找的值
     * @return Node* 第一个 >= val 的节点，不存在返回 nullptr
     * 时间复杂度：O(log n)
     */
    ...
    /**
     * @brief 查找第一个 > val 的节点（upper_bound）
     * @param val 要查找的值
     * @return Node* 第一个 > val 的节点，不存在返回 nullptr
     * 时间复杂度：O(log n)
     */
...
public:
    ...
    /**
     * @brief 返回指向最后一个元素的迭代器（最大值）
     * @return Iterator 指向最大元素的迭代器
     * 时间复杂度：O(log n)
     * 
     * 注意：这里使用同一迭代器类型实现反向遍历
     * 实际使用时应为 reverse_iterator 适配器
     */
    ...
    /**
     * @brief 返回第一个 >= val 的迭代器
     * @param val 要查找的值
     * @return Iterator 指向第一个 >= val 的元素
     * 时间复杂度：O(log n)
     */
 ...
 */
template<typename T>
void swap(MyMultiset<T>& a, MyMultiset<T>& b) {
    a.swap(b);
}